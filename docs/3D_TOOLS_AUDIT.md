...
- Tests: [apps/web/src/stores/**tests**/featureStore.test.ts](apps/web/src/stores/__tests__/featureStore.test.ts), [apps/web/src/lib/cad/**tests**/cadWorkerClient.test.ts](apps/web/src/lib/cad/__tests__/cadWorkerClient.test.ts)
...
| 5   | **Fillet**          | ✅                    | `FilletDialog`          | picker en `ToolbarModifiers` | `createFillet`            | `fillet`             | `executeFillet`            | ✅ (Fillet + errores)    | ❌                                                     | Soporta selección por arista; sin selección usa todas las aristas     |
| 6   | **Chamfer**         | ✅                    | `ChamferDialog`         | picker en `ToolbarModifiers` | `createChamfer`           | `chamfer`            | `executeChamfer`           | ✅ (Chamfer + errores)   | ❌                                                     | Igual que Fillet; flujo principal en `ToolbarModifiers`               |
...
### 4.3 ⚠️ Fillet/Chamfer tienen selección por arista, pero con discoverability mejorable

Archivos: `ToolbarModifiers.tsx`, `Canvas3D.tsx`, `uiStore.ts`, `executeFillet` y `executeChamfer` en `cad.worker.ts`.

El flujo principal ya soporta selección de aristas desde el viewport y envía `edgeIndices` al worker. El problema actual no es la ausencia de la capacidad, sino que todavía hay documentación y textos heredados que inducen a pensar que el comportamiento sigue siendo global. El worker mantiene además un fallback válido: si no hay selección explícita, aplica sobre todas las aristas.
...
[docs/EXTRUSION_3D.md](docs/EXTRUSION_3D.md) (v0.2.0) lista Revolve, Fillet y Chamfer en _“Próximas Mejoras”_, pero los tres están implementados con tests. Reescribir en línea con este audit o redirigir desde allí a este informe.
...
| **P2**    | Pulir discoverability, validaciones y feedback del flujo de selección de aristas para Fillet/Chamfer (§4.3)                                                                                                                                               | Medio   | M        |
